-- USER_PRODUCT e ITEM sem ficha (name is null -- ML bloqueia pra
-- terceiro, ja confirmado) apareciam na busca so com o codigo MLB cru.
-- Usuario pediu pra remover em vez de mostrar vazio -- os proximos da
-- mesma posicao/categoria ja entram sozinhos no lugar (a query nao
-- limitava por produto, so por linha; tirar os sem nome so libera
-- espaco pro resto do mesmo resultado). Resto copiado exato de
-- 20260725070000_arvore_categorias.sql, so acrescenta "and v.name is
-- not null".
